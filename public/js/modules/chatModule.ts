...
import { ChatMessage } from '../types'; // Use extensionless path, removed unused UserData, PersonaVariation
...
export function initChatModule(elements: ChatModuleElements): void {
    ...
    // Listen for personality generated event to update prompt
     document.addEventListener('personality-generated', (event: Event) => {
        const customEvent = event as CustomEvent;
...
export async function sendChatMessage(): Promise<void> {
    if (!chatInputElement || !chatHistoryDiv || !chatStatusDiv) return;
...
async function saveChatPromptVariation(): Promise<void> {
    if (!systemPromptEditor || !state.currentUserId) return;
