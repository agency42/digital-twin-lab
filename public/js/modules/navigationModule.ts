...
// Define a type for the relevant elements passed from app.ts
// We only need navTabs and pages based on the usage in this module
interface NavigationElements {
    navTabs: NodeListOf<Element> | null;
    pages: NodeListOf<Element> | null;
}

// Module state/cache
let navTabs: NodeListOf<Element> | null = null;
let pageContainers: NodeListOf<Element> | null = null;

/**
 * Initialize the navigation module
 * @param elements - UI elements containing navTabs and pages
 */
export function initNavigationModule(elements: NavigationElements): void {
    navTabs = elements.navTabs;
    pageContainers = elements.pages;

    console.log('Navigation module initialized with', 
        navTabs ? navTabs.length : 0, 'tabs and', 
        pageContainers ? pageContainers.length : 0, 'pages');
 ...
 */
function handleNavTabClick(event: Event): void {
    ...
    const contentLibraryStatusDiv = document.getElementById('content-library-status') as HTMLDivElement | null;
    ...
    if (targetPageId === 'content-library-page') {
        ...
        if (state.currentUserId || hasUserInUI) {
            ...
            showStatus(contentLibraryStatusDiv ?? document.createElement('div'), 
                       'Please select a user first to access the Content Library', 'error');
            return;
        }
    }
 ...
 */
export function handlePageTransition(pageId: string): void {
...
export function navigateToPage(pageId: string): void {
    const tab = document.querySelector(`.nav-tab[data-page="${pageId}"]`) as HTMLElement | null;
