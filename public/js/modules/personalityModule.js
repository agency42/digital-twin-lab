...
let personalityPromptTextarea = null;
let savePromptButton = null;
let resetPromptButton = null;
let promptStatusDiv = null;
let generatePersonalityButton = null;
let personalityGenerationStatusDiv = null;
let personalityJsonOutputPre = null;
let copyJsonButton = null;
let primaryPersonaDisplayContainer = null;
let noPrimaryPersonaMessage = null;
 ...
 */
export function initPersonalityModule(elements) {
    ...
    document.addEventListener('content-library-page-activated', (event) => {
        const customEvent = event;
    ...
    document.addEventListener('assets-selection-changed', (event) => {
        const customEvent = event;
 ...
 */
export async function loadPersonalityPrompt() {
 ...
 */
export function getDefaultPersonalityPrompt() {
 ...
 */
export function resetPersonalityPrompt() {
    if (!personalityPromptTextarea)
        return;
    const defaultPrompt = getDefaultPersonalityPrompt();
    personalityPromptTextarea.value = defaultPrompt;
...
export async function generatePersonalityProfile() {
    if (!generatePersonalityButton || !personalityGenerationStatusDiv)
        return;
    ...
    try {
        ...
        if (generatePersonalityButton)
            generatePersonalityButton.disabled = false;
    }
}
/**
 * Copy the generated personality JSON to clipboard
 */
export function copyGeneratedJson() {
 ...
 */
function displayPrimaryPersona() {
 ...
 */
export function createProfileCard(profile, timestamp) {
    ...
    if (!card)
        throw new Error('Invalid profile card template: Missing .profile-card element');
    // Set title and date
    const titleEl = card.querySelector('.profile-card-title');
    const dateEl = card.querySelector('.profile-card-date');
    if (titleEl)
        titleEl.textContent = 'Primary Personality Profile'; // Static title for now
    if (dateEl)
        dateEl.textContent = `Updated: ${new Date(timestamp).toLocaleString()}`;
    // Add trait badges
    ...
    if (traitsEl && profile.big_five_traits) {
        ...
        for (const [trait, level] of Object.entries(profile.big_five_traits)) {
            if (typeof level !== 'string')
                continue; // Skip if level is not a string
            ...
            if (level.toLowerCase() === 'high')
                badge.style.cssText = 'background-color:#d4edda; color:#155724;';
            else if (level.toLowerCase() === 'low')
                badge.style.cssText = 'background-color:#f8d7da; color:#721c24;';
            else
                badge.style.cssText = 'background-color:#e0e0e0; color:#333;';
...
//# sourceMappingURL=personalityModule.js.map