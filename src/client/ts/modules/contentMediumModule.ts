...
export function initContentMediumModule(): void {
    ...
    generateContentButton = document.getElementById(
    ...
    showSystemPromptCheckbox = document.getElementById(
        'show-system-prompt-checkbox'
    ) as HTMLInputElement | null;

    // ADD selectors for new elements
    systemPromptStatusDiv = document.getElementById(
    ...
    if (!mediumTabsContainer)
        console.error(
            'ContentMediumModule Error: Could not find mediumTabsContainer (.medium-tabs)'
        );
    if (!systemPromptEditor)
        console.error(
            'ContentMediumModule Error: Could not find systemPromptEditor (#system-prompt-editor)'
        );
    if (!instructionEditor)
        console.error(
            'ContentMediumModule Error: Could not find instructionEditor (#instruction-editor)'
        );
    if (!resetPromptButton)
        console.error(
            'ContentMediumModule Error: Could not find resetPromptButton (#reset-prompt-button)'
        );
    if (!generateContentButton)
        console.error(
            'ContentMediumModule Error: Could not find generateContentButton (#generate-content-button)'
        );
    if (!generatedContentOutput)
        console.error(
            'ContentMediumModule Error: Could not find generatedContentOutput (#generated-content-output)'
        );
    if (!generationStatusDiv)
        console.error(
            'ContentMediumModule Error: Could not find generationStatusDiv (#generation-status)'
        );
    if (!showSystemPromptCheckbox)
        console.error(
            'ContentMediumModule Error: Could not find showSystemPromptCheckbox (#show-system-prompt-checkbox)'
        );
    if (!systemPromptStatusDiv)
        console.error(
            'ContentMediumModule Error: Could not find systemPromptStatusDiv (#system-prompt-status)'
        );
    if (!instructionStatusDiv)
        console.error(
            'ContentMediumModule Error: Could not find instructionStatusDiv (#instruction-status)'
        );
    if (!saveSystemPromptButton)
        console.error(
            'ContentMediumModule Error: Could not find saveSystemPromptButton (#save-system-prompt-button)'
        );
    ...
    if (!examplesStatusDiv)
        console.error(
            'ContentMediumModule Error: Could not find examplesStatusDiv (#examples-status)'
        );
    // --- End Element Checks ---
    ...
    if (mediumTabsContainer) {
        ...
        console.error(
            'ContentMediumModule Error: Cannot add tab listeners because mediumTabsContainer was not found.'
        );
    }
    ...
    document.addEventListener('user-data-loaded', () => {
        console.log(
            'ContentMediumModule: User data loaded, fetching initial generations data for',
            currentMedium
        );
    ...
    document.addEventListener('character-card-updated', (event: Event) => {
        ...
        console.log(
            "%cContentMediumModule: Received 'character-card-updated' event!",
        ...
        if (customEvent.detail && customEvent.detail.cardData) {
            ...
            if (systemPromptEditor && systemPromptEditor.getAttribute('data-is-custom') === '0') {
                console.log(
                    'ContentMediumModule: Current prompt was default, updating editor with new card data.'
                );
                ...
                showStatus(
                    systemPromptStatusDiv,
                    'System prompt updated to match new character card.',
                    'info',
                    3000
                );
            } else {
                console.log(
                    'ContentMediumModule: Current prompt was custom, leaving editor as is. Reset button available.'
                );
                // Optionally: Notify user that a new base card is available and they can reset
                showStatus(
                    systemPromptStatusDiv,
                    'New character card generated. You can reset the system prompt if desired.',
                    'info',
                    5000
                );
            }
            ...
            console.warn(
                "ContentMediumModule: 'character-card-updated' event received, but no cardData in detail."
            );
        }
    });
...
async function fetchAndLoadGenerationsData(userId: string, type: ContentMediumType): Promise<void> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                errorData.error || `Failed to fetch generations data (${response.status})`
            );
        }
        ...
        if (systemPromptEditor) {
            systemPromptEditor.value =
                data.systemPrompt?.prompt_text ?? data.characterCard?.card_data ?? '';
            // Store is_custom status and current character card data for comparison/reset
            systemPromptEditor.setAttribute(
                'data-is-custom',
                (data.systemPrompt?.is_custom ?? 0).toString()
            );
            ...
            if (resetPromptButton) {
                resetPromptButton.disabled =
                    !currentCharacterCardData || !data.systemPrompt?.is_custom;
            }
        } else {
...
function switchMedium(newMedium: ContentMediumType): void {
    ...
    if (!contentGenerationArea)
        console.error('switchMedium ERROR: Could not find content-generation-area element');
    if (!mediumContentArea)
        console.error('switchMedium ERROR: Could not find medium-content-area element');
...
async function handleSaveSystemPrompt(): Promise<void> {
    ...
    try {
        ...
        showStatus(
            systemPromptStatusDiv,
            `${type.charAt(0).toUpperCase() + type.slice(1)} system prompt saved successfully.`,
            'success',
            3000
        );
    } catch (error: any) {
...
async function handleResetSystemPrompt(): Promise<void> {
    if (!state.currentUserId || !resetPromptButton || !systemPromptEditor || !systemPromptStatusDiv)
        return;
    ...
    if (
        !confirm(
            `Are you sure you want to reset the ${type} system prompt to the current character card?`
        )
    ) {
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response
                ...
                .catch(() => ({ error: `HTTP error ${response.status}` }));
            throw new Error(
                errorData.error || `Failed to reset system prompt (${response.status})`
            );
        }
        ...
        showStatus(
            systemPromptStatusDiv,
            `${type.charAt(0).toUpperCase() + type.slice(1)} system prompt reset successfully.`,
            'success',
            3000
        );
    } catch (error: any) {
...
async function handleSaveInstructions(): Promise<void> {
    ...
    try {
        ...
        showStatus(
            instructionStatusDiv,
            `${type.charAt(0).toUpperCase() + type.slice(1)} instructions saved successfully.`,
            'success',
            3000
        );
    } catch (error: any) {
...
async function generateContent(): Promise<void> {
    ...
    try {
        ...
        generatedContentOutput.innerHTML = `
            <h4>Generated ${
                currentMedium.charAt(0).toUpperCase() + currentMedium.slice(1)
            } Content</h4>
