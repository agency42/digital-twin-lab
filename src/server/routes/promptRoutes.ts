...
function createPromptRouter(abstractionApproach: AbstractionApproach): Router {
    ...
    router.post(
        '/:userId/generate-character-card',
        asyncHandler(async (req: Request, res: Response) => {
            ...
            if (!userId || !assetIds || !Array.isArray(assetIds) || assetIds.length === 0) {
                return res
                    .status(400)
                    .json({ error: 'Missing required fields: userId and assetIds array.' });
            }

            // 1. Generate the character card content
            const characterCardString = await abstractionApproach.generateCharacterCard(
                userId,
                assetIds,
                customPrompt
            );
    ...
    router.get(
        ...
        asyncHandler(async (req: Request, res: Response) => {
            ...
            if (card) {
                ...
                return res
                    .status(404)
                    .json({ error: 'Current character card not found for this user.' });
            }
        })
    );
    ...
    router.get(
        ...
        asyncHandler(async (req: Request, res: Response) => {
            ...
            if (type !== 'chat' && type !== 'post') {
                return res
                    .status(400)
                    .json({ error: "Invalid type parameter (must be 'chat' or 'post')" });
            }
            ...
            try {
                ...
                return res
                    .status(500)
                    .json({ error: `Failed to get generations data: ${error.message}` });
            }
        })
    );
    ...
    router.put(
        ...
        asyncHandler(async (req: Request, res: Response) => {
            ...
            if (typeof promptText !== 'string') {
                return res
                    .status(400)
                    .json({ error: "Missing or invalid 'promptText' in request body." });
            }
    ...
    router.post(
        '/:userId/system-prompts/:type/reset',
        asyncHandler(async (req: Request, res: Response) => {
            const { userId, type } = req.params as { userId: string; type: 'chat' | 'post' };
            ...
            if (resetPrompt) {
                ...
                return res
                    .status(404)
                    .json({ error: 'Could not reset prompt, character card may be missing.' });
            }
        })
    );
    ...
    router.put(
        '/:userId/instruction-templates/:type',
        asyncHandler(async (req: Request, res: Response) => {
